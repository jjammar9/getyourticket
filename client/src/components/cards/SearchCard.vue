...
<template>
  ...
  >
    <div class="relative h-[175px] overflow-hidden">
      ...
      <button @click.stop="toggleWishlist" class="absolute top-3 right-3 w-10 h-10 bg-white rounded-full shadow-sm flex items-center justify-center z-10">
        <Heart :size="18" stroke-width="2.3" :fill="authStore.isInWishlist(item.id) ? 'currentColor' : 'none'" :class="authStore.isInWishlist(item.id) ? 'text-[#e53935]' : 'text-[#0b2343]'" />
      </button>
