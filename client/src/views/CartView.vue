  ...
  <Container>
    <div class="pt-32 pb-12">
      ...
      <div v-if="confirmSuccess" class="text-center py-20">
        ...
        <p class="text-[18px] font-semibold text-[#0b2343] mb-4">Booking confirmed!</p>
      ...
      <div v-else-if="cartItems.length === 0" class="text-center py-20">
        <ShoppingCart :size="48" class="mx-auto text-gray-300 mb-4" />
        <p class="text-[16px] text-gray-500 mb-4">{{ localeStore.t("cart.empty") }}</p>
      ...
      <div v-else class="grid grid-cols-1 lg:grid-cols-3 gap-10">
        <div class="lg:col-span-2 space-y-4">
          <div
            ...
            :key="index"
            class="flex gap-4 bg-white rounded-[18px] border border-[#d9dee8] p-4"
          >
            <img
              :src="item.image"
              :alt="item.title"
            ...
            <div class="flex-1 min-w-0">
              <h3 class="text-[15px] font-extrabold text-[#0b2343] truncate">{{ item.title }}</h3>
              <p class="text-[18px] font-extrabold text-[#e53935] mt-1">{{ currencyStore.formatPrice(item.price) }}</p>
              <div class="flex items-center gap-4 mt-2">
                <div>
                  <label class="text-[12px] font-medium text-gray-500 block">{{ localeStore.t("cart.guests") }}</label>
                  <input
                    type="number"
                    min="1"
                    :value="item.guests || 1"
                    @input="updateGuests(index, $event.target.value)"
                    class="w-16 mt-1 px-2 py-1 border border-[#d9dee8] rounded-lg text-[14px] text-[#0b2343]"
                  />
                </div>
              </div>
            </div>
            <button
              ...
              class="shrink-0 w-8 h-8 flex items-center justify-center rounded-full hover:bg-red-50 transition-colors"
            >
        ...
        <div class="bg-white rounded-[18px] border border-[#d9dee8] p-6 h-fit">
          <h2 class="text-[18px] font-bold text-[#0b2343] mb-4">{{ localeStore.t("cart.checkout") }}</h2>

          <div class="space-y-3">
            <div>
              <label class="text-[13px] font-medium text-[#59657b]">{{ localeStore.t("cart.name") }}</label>
              <input
                v-model="formName"
                type="text"
            ...
            <div>
              <label class="text-[13px] font-medium text-[#59657b]">{{ localeStore.t("cart.email") }}</label>
              <input
                v-model="formEmail"
                type="email"
                class="w-full mt-1 px-3 py-2 border border-[#d9dee8] rounded-lg text-[14px] text-[#0b2343] focus:outline-none focus:border-[#ff5533]"
            ...
            <div>
              <label class="text-[13px] font-medium text-[#59657b]">{{ localeStore.t("cart.date") }}</label>
              <input
                v-model="formDate"
                type="date"
                class="w-full mt-1 px-3 py-2 border border-[#d9dee8] rounded-lg text-[14px] text-[#0b2343] focus:outline-none focus:border-[#ff5533]"
          ...
          <div class="mt-6 pt-4 border-t border-[#d9dee8]">
            <div class="flex justify-between items-center">
              <span class="text-[16px] font-semibold text-[#0b2343]">{{ localeStore.t("cart.total") }}</span>
